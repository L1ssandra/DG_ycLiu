    ...
    do i = 1,Nx
        do j = 1,Ny
            call eigenvalueMm(alpha1,alpha2,uh(i,j,1,1),uh(i,j,1,2),uh(i,j,1,3),uh(i,j,1,4),uh(i,j,1,5),uh(i,j,1,6),1,0)
            if (abs(alpha1) > alphax .or. abs(alpha2) > alphax) then
                alphax = max(abs(alpha1),abs(alpha2))
            end if
            call eigenvalueMm(alpha1,alpha2,uh(i,j,1,1),uh(i,j,1,2),uh(i,j,1,3),uh(i,j,1,4),uh(i,j,1,5),uh(i,j,1,6),0,1)
